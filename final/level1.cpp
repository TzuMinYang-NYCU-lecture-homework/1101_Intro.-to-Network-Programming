...
{
    if(argc < 3) err_quit("Usage: ./level1 token1 10100");

    tcp_connect(argv[1], argv[2]);
    tcp_read_until_and_stdout("Enter your UDP port:");

    udp_lookup(argv[1], argv[2]);
    udp_bind();

    str_tcp_output_buf = to_string(UDP_PORT);
    tcp_write_and_stdout();

    tcp_read_until_and_stdout("received?");

    udp_recv_and_stdout();
    str_tcp_output_buf = str_udp_input_buf;
    tcp_write_and_stdout();
